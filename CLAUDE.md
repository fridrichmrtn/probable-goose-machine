...
- Application runtime LLM: default to OpenRouter via the OpenAI-compatible API. `google/gemini-3.1-flash-lite` is primary for the `cheap`, `extract`, and `vision` slots with `google/gemini-3.5-flash` as the per-slot fallback. The `reasoning` slot (growth stage) keeps `google/gemini-3.5-flash` as primary with Flash-Lite as the fallback, because anchor-verified action generation needs the more capable model. (Re-pinned from the 2.5 family on 2026-06-12 after OpenRouter delisted those slugs; the tier intent — Flash-Lite for cheap/extract/vision, full Flash for reasoning — is unchanged. The `live`-marked catalog-guard test fails CI if a configured slug stops resolving.)
- An optional non-paid `local` provider (Ollama / self-hosted, OpenAI-compatible) can be opted in per slot via `GANDER_LLM_PROVIDER_<SLOT>=local` (`GANDER_LOCAL_BASE_URL` defaults to `http://localhost:11434/v1`). It is OFF by default; OpenRouter stays primary for every slot. Vision always uses OpenRouter (a `vision=local` override degrades back to OpenRouter), and self-hosted calls estimate to ~0 cost.
