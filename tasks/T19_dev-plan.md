...
Deliver `tests/test_confidence_judge.py` with the 6 tests from `tasks/T19_judge_tests.md` §Deliverables — 2 `@pytest.mark.fast`, 4 `@pytest.mark.live`. Extract a small `_render_step_b()` helper out of `src/gander/confidence.py::judge()` so the Step B prompt rendering can be unit-tested for leakage without an LLM call. No behaviour change to Step A, telemetry, regenerate-on-Low retry loop, or fallback paths.

## Files to modify

### `src/gander/confidence.py`

Extract the Step B user-message string into a pure module-level function. The current code (lines 108-110 of `src/gander/confidence.py`) inlines it inside `judge()`:
...
Leading-underscore name is intentional: matches the existing convention (`_TierOnly`, `_LOW_FALLBACK_RATIONALE`, `_FAILURE_MSG`) and T12 tests already import private symbols from this module — so the import path `from gander.confidence import _render_step_b` works.

## Files to create

### `tests/test_confidence_judge.py`

Style matches `tests/test_confidence_unit.py`: `from __future__ import annotations`, `pytest` import, `Source` / `judge` / `Confidence` from `gander`, a small `_sources_*` helper per scenario. Live tests use `os.environ.get("MINIMAX_API_KEY")` skipif (pattern from `tests/test_salary.py:299-302`).
...
2. **`test_step_b_does_not_see_estimator_reasoning`** — `@pytest.mark.fast`. No mocks. Imports `_render_step_b` from `gander.confidence`. Calls it with representative args (e.g. `("Low", 100000, 200000, "CZK", "month")`). Lowercases the result. Asserts each of `"estimator"`, `"reasoning"`, `"profile"` is not in the lowered string. Single function call, three asserts.
...
- **No new runtime dependencies.** All imports are stdlib (`inspect`, `os`, `re`) plus existing `pytest` and `gander.*`.
- **mypy strict on src only** (`pyproject.toml:41` — `files = ["src/gander"]`). The test file is not type-checked; no strict-mode burden.
