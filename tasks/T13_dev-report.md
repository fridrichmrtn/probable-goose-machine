...
- `src/gander/prompts/growth.md` — not modified this run; ban-list bullet at line 34 verified to survive verbatim.
...
|---|---|---|
| `uv run ruff format --check src/gander/growth.py tests/test_growth_unit.py` | pass | pass |
| `uv run ruff check src/gander/growth.py tests/test_growth_unit.py` | pass | pass |
| `uv run mypy src/gander/growth.py` | pass | pass |
| `uv run pytest -q -m fast tests/test_growth_unit.py` | pass (9) | pass (22) |
| `uv run pre-commit run --all-files` | pass | pass |

(`ruff format --check` does not check `.md`; the prompt file is verified by `grep` for the surviving ban-list bullet at `src/gander/prompts/growth.md:34`.)

## Review findings

### Must-fix (resolved this run, single heal iteration)

- **[ai-ml-engineer]** `_check_ban_phrase` `src/gander/growth.py:100-108` — added punctuation strip on the lowercased haystack so `"Ph.D."` → `"phd"` matches; new fast test `test_plan_growth_drops_ban_phrase_phd_dotted` pins the exact `"Complete a Ph.D. in ML"` case.
- **[ai-ml-engineer]** `growth_actions_truncated` event `src/gander/growth.py:232-239` — emit only when `len(survivors) > 5` with `count_before`/`count_after`/`dropped`; positive-path test asserts 7→5 fires correctly, two new boundary tests (3 and 5 survivors) assert it does NOT fire.
- **[product-owner]** Top-level `try/except Exception` `src/gander/growth.py:144-273` — wraps the entire stage body so any escape (e.g., a `verify_quote` crash) surfaces PRD §4.6 `_FAILURE_MSG` with `stage_failure.reason="unexpected_error"`. The pre-existing fine-grained excepts on `complete_json` are preserved. New test `test_plan_growth_returns_stage_failure_on_unexpected_error` monkeypatches `growth_mod.verify_quote` to raise and pins both the user-message and the structured reason.
...
- **[hiring-manager]** Temperature divergence `src/gander/growth.py:150` — kept at `0.0` (consistent with T10/T11/T12) with the comment `# temperature=0.0 for determinism — matches T10/T11/T12 stages.` immediately above the `complete_json` call.
- **[hiring-manager]** Deterministic truncation rationale `src/gander/growth.py:240` — comment added: `# Order preserved from the model's emitted list — prompt instructs "strongest-first".`
...
4 minor: (1) the unreachable `return StageFailure(...)` after the `async with` block exists solely for mypy — a comment justifies it; (2) `_BAN_PHRASES` substring-matches inside longer English words is intentional and documented in the module docstring; (3) `_BASELINE_PATH` resolves via `parents[2]` which is fragile if the file is ever moved out of `src/gander/` (deliberate, simplest path resolution); (4) the parametrized ban-phrase test reuses the same three non-banned actions in every parameter case — extracting a helper would tighten the test code but the duplication aids readability of what each case is exercising.
