...
- [x] `pyproject.toml` updates — confirm `[tool.ruff]` line-length 100, target-version `py311`; `[tool.mypy]` strict mode on `src/gander/*`; `[tool.pytest.ini_options]` markers declared. (Verified already-configured by plan §0; no edits needed.)
