...
- [x] `src/jobfit/errors.py` (extension): wire `stage_boundary` to call `obs.emit("error", stage=..., exc_type=..., exc_message=...)` on catch. Set the stage name in a `contextvars.ContextVar` so `llm.py` knows which stage to attribute its calls to.
