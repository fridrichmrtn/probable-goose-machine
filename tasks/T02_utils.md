...
- [x] `src/gander/llm.py`:
  ...
  - Constructor reads `GANDER_MODEL_PROFILE` env var: `local` (default) → M1; `ci` → `abab6.5s-chat`. Each stage requests a model via a logical name (`reasoning` / `cheap`) which the profile resolves.
