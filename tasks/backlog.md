...
- [ai-ml-engineer] `Source.domain` not validated against `]` — `src/jobfit/schemas.py` — `Source.domain` is a free-form `str`; a value containing `]` would visually fake the bracket close in the rendered `[domain] — "snippet"` source line. Low risk (set by the salary stage, not user input) but a one-line validator (`"]" not in domain`) is cheap. Cross-task: belongs to T01 (schemas) or T11 (salary).
